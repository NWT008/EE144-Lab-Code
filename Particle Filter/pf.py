...
#                       PF PARAMETERS
# ============================================================================
N_PARTICLES = 500        # M, number of particles
INIT_XY_SPREAD = 1.0     # m, half-width of initial uniform spread
INIT_THETA_SPREAD = math.pi  # rad, initial uniform yaw spread
...
class ParticleFilter:
    ...
    def predict(self, v, omega, dt):
        ...
        for m in range(N_PARTICLES):
            v_m = v + np.random.normal(0, self.sigma_v)
            w_m = omega + np.random.normal(0, self.sigma_w)
    ...
    def update(self, detections):
        ...
        for (r, b) in detections:
            ...
            for m in range(N_PARTICLES):
                ...
                log_likelihood = -0.5 * (nu_m.T @ self.R_inv @ nu_m)

                log_w[m] += log_likelihood
    ...
    def resample(self):
        ...
        for i in range(M):
            ...
            while threshold_i > cumulative[j]:
                j += 1
