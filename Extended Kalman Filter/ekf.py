...
# ============================================================================
SIGMA_V       = 0.1    # m/s   -- linear velocity noise
SIGMA_W       = 0.1    # rad/s -- angular velocity noise
SIGMA_RANGE   = 0.20    # m     -- LIDAR range noise
SIGMA_BEARING = 0.1    # rad   -- LIDAR bearing noise
...
class EKFLocalization:
    ...
    def predict(self, v, omega, dt):
        ...
        if abs(omega) < 1e-6:
            ...
            dtheta = 0.0
             ...
             dx = (v / omega) * (-np.sin(theta) + np.sin(theta + omega * dt))
        ...
        if abs(omega) < 1e-6:
            F_x[0,2] = -v * dt * np.sin(theta)
            F_x[1,2] = v * dt * np.cos(theta)

            F_u[0,0] = dt * np.cos(theta)
            F_u[1,0] = dt * np.sin(theta)
            F_u[2,1] = dt

        else:
            F_x[0,2] = (v / omega) * (-np.cos(theta) + np.cos(theta + omega * dt))
            F_x[1,2] = (v / omega) * (-np.sin(theta) + np.sin(theta + omega * dt))

            F_u[0,0] = (-np.sin(theta) + np.sin(theta + omega *dt)) / omega
            F_u[0,1] = (v * (np.sin(theta) - np.sin(theta + omega *dt)) / (omega ** 2) + v * dt * np.cos(theta + omega *dt) / omega)

            F_u[1,0] = (np.cos(theta) - np.cos(theta + omega * dt)) / omega
            F_u[1,1] = (
			v * (-np.cos(theta) + np.cos(theta + omega * dt)) / (omega ** 2) 
    ...
    def update(self, z, landmark_xy):
        ...
        z_hat = np.array([np.sqrt(q), wrap_to_pi(np.arctan2(dy,dx) - theta)])
        ...
        #     mu = mu + K @ y_innov
        # ----------------------------------------------------------------
        self.mu = self.mu + K @ y_innov # <-- TODO: implement Eq. (5c)
        self.mu[2] = wrap_to_pi(self.mu[2])
        ...
        # ----------------------------------------------------------------
        I = np.eye(3)
        self.Sigma = (I - K @ H) @ self.Sigma  # <-- TODO: implement Eq. (5d)
